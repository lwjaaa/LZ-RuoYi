...
- **节点渲染 (`slot`)**:
  - **显示格式**: `Label + Value`。其中Label中文，value是英文。
...
- **拖拽排序 (`TagEditDialog`)**:
  ...
  - **约束逻辑**:
    - `type === 'MENU'` 的节点之间才有上下级关系，且第一级别的 tag 永远排在其他类型 tag 的前面。其他类型的节点只能在第一层级，没有上下级关系。
    - 非 MENU 节点拖拽或点击置顶到 MENU 节点之前时，自动修改排序为该 MENU 节点之后的第一个位置。
    - `type === 'MENU'` 的节点之间允许跨层级拖拽（改变 `parentId`），但是不能拖拽到其他类型节点的后面。
    - 其他类型的的节点只能拖拽到 MENU 节点下，且不能跨层级。
