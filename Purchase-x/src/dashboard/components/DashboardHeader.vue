<template>
  <header class="sticky top-0 z-50 bg-glass-heavy backdrop-blur-glass-heavy border-b border-white/10">
    <div class="neon-line" />
    <div class="px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
      <div class="flex items-center gap-3 flex-shrink-0">
        <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-accent to-purple flex items-center justify-center shadow-glow">
          <svg class="w-5 h-5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" />
          </svg>
        </div>
        <h1 class="text-lg font-bold bg-gradient-to-r from-accent via-purple-light to-cyan bg-clip-text text-transparent whitespace-nowrap">
          PurchaseX
        </h1>
      </div>

      <div class="hidden md:flex items-center gap-1 bg-primary-dark/60 rounded-xl p-1 border border-white/5">
        <button
          :class="[
      ...
      <div class="flex items-center gap-2 overflow-x-auto">
        ...
        <button
          ...
          @click="$emit('sync')"
        >
          <svg class="w-3.5 h-3.5 inline-block mr-1 -mt-0.5" :class="{ 'animate-spin-slow': isSyncing }" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
