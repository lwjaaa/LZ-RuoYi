...
class TaobaoExtractor extends BaseExtractor {
  constructor() {
    super();
  ...
  async extractOptionList() {
    ...
    try {
      ...
      if (resData && resData.skuBase && resData.skuBase.props) {
        ...
        for (const prop of resData.skuBase.props) {
          ...
          const englishName = generateEnglishName(chineseName, "taobao");
    ...
    try {
      ...
      Logger.info(`找到 ${skuItems.length} 个 SKU 维度`);
  ...
  _parsePriceFromSkuInfo(skuInfo) {
    ...
    try {
      ...
      if (priceObj && priceObj.priceText) {
        const parsed = extractPriceFromText(priceObj.priceText, "taobao");
  ...
  async extractProductVariantList(optionList) {
    Logger.step("商品变体", "正在提取商品变体信息...optionList");
    const resData = this._getResData();
    const hasScriptData = !!(
      resData &&
      resData.skuCore &&
    ...
    if (!skuArea) {
      Logger.warn("未找到 SKU 区域，无法遍历规格组合");
      return variantList;
    }
    ...
    for (let i = 0; i < shuffledCombinations.length; i++) {
      ...
      if (hasDisabled) {
        Logger.info(`  组合包含禁用规格，跳过选择，记录空数据`);
  ...
  _extractVariantsFromScript(optionList, resData) {
    ...
    for (const combo of combinations) {
      ...
      const quantity = skuInfo ? parseInt(skuInfo.quantity) || 0 : 0;
      console.log("quantity", quantity);

      const isActiveAvailable = quantity > 0 ? 1 : 0;
      ...
      if (combo.length > 0) {
        for (let i = combo.length - 1; i >= 0; i--) {
          if (combo[i].imageUrl) {
            mediaUrl = combo[i].imageUrl;
            break;
          }
        }
      }
      console.log("isActiveAvailable", isActiveAvailable);
      variantList.push({
        purchaseUrl: window.location.href,
        purchasePrice: price,
        mediaUrl: mediaUrl,
    ...
    Logger.success(
      "商品变体 [Script]",
      `提取到 ${variantList.length} 个变体（直接从 JSON 解析）`,
    );
    return variantList;
  }
  ...
  async getCurrentPrice() {
    try {
      const resData = this._getResData();
      if (resData && resData.skuCore && resData.skuCore.sku2info) {
        const defaultSkuInfo = resData.skuCore.sku2info["0"];
        if (defaultSkuInfo) {
          const price = this._parsePriceFromSkuInfo(defaultSkuInfo);
          if (price !== null && price > 0) {
            Logger.info("当前价格 [Script]", price);
            return price;
          }
        }
      }
    } catch (e) {
      Logger.warn("Script 获取价格失败，回退到 DOM", e);
    }
  ...
  async getCurrentMainImage() {
    try {
      ...
      if (
        ...
        const url = normalizeImageUrl(resData.item.images[0]);
        if (url) return url;
      }
    } catch (e) {
      Logger.warn("Script 获取主图失败，回退到 DOM", e);
    }
...
if (typeof window !== "undefined") {
  window.TaobaoExtractor = TaobaoExtractor;
}
